...
@pytest.mark.parametrize("ref_fraction,number_of_repetitions", [(0.5, 1), (0.6,1), (0.8,1), (1.0,1)
                                                            ,(0.5, 10), (0.6,10), (0.8,10), (1.0,10)
                                                            ,(0.5, 50), (0.6,50), (0.8,50), (1.0,50),
                                                            ],)                                                                                             
def test_schema_ref_option_parquet(cellpainting,ref_fraction, number_of_repetitions):
    ...
    reference_dicts = {}
    reference_dicts["ref_sampling"] = get_reference_dicts(data=cellpainting, ref_fraction=ref_fraction, number_of_repetitions=number_of_repetitions)  # path to config file with "reference_option = sample"
    reference_dicts["ref_chosen_file"] = get_reference_dicts(data=cellpainting)# path to config file with "reference_option = path/to/reference_file"
  
    for blob in ingest:  # iterate over compartment types
        compartment_name = blob["table"]
        predetermined_schema = reference_dicts["ref_chosen_file"][compartment_name.capitalize()]["schema"]
