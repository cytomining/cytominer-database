...
def __main__(a, b):
    ...
    for pattern in patterns:
        ...
        object_identifiers = data['ImageNumber'].unique()

        create_objects(data, image_and_object_identifiers, object_identifiers, session)

        object_identifiers = data['ObjectNumber']

        for object_identifier in object_identifiers:
            match = Match(object_id=int(object_identifier))

            session.add(match)

        for identifier in image_and_object_identifiers:
            print(identifier)

        # center = Coordinate(abscissa=int(round(row['Location_Center_X'])), ordinate=int(round(row['Location_Center_Y'])))
...
def create_objects(data, image_and_object_identifiers, object_identifiers, session):
    for image_identifier in object_identifiers:
        object_identifiers = data[data['ImageNumber'] == image_identifier]['ObjectNumber']

        for object_identifier in object_identifiers:
            Object.create_by(session=session, id=int(object_identifier), image_id=int(image_identifier))

            image_and_object_identifiers.append((image_identifier, object_identifier))
